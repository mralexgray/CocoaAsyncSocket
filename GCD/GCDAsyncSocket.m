...
NSString *const GCDAsyncSocketException = @"GCDAsyncSocketException",
			*const GCDAsyncSocketErrorDomain = @"GCDAsyncSocketErrorDomain";

#if !TARGET_OS_IPHONE
NSString *const GCDAsyncSocketSSLCipherSuites = @"GCDAsyncSocketSSLCipherSuites",
			*const GCDAsyncSocketSSLDiffieHellmanParameters = @"GCDAsyncSocketSSLDiffieHellmanParameters";
#endif
...
- (void)ensureCapacityForAdditionalDataOfLength:(NSUInteger)bytesToRead{
	...
	NSUInteger buffSize 	=       buffer.length,
				  buffUsed 	= startOffset + bytesDone,
				  buffSpace = buffSize    -  buffUsed;
	if (bytesToRead > buffSpace)	[buffer increaseLengthBy:bytesToRead - buffSpace];
}
...
- (NSUInteger)readLengthForTermWithPreBuffer:(NSData *)preBuffer found:(BOOL *)foundPtr{
	...
	if ((bytesDone + preBufferLength) < termLength)
		return preBufferLength;	// Not enough data for a full term sequence yet

	// Note: maxLength >= termLength
	NSUInteger maxPreBufferLength = maxLength ? MIN(preBufferLength, (maxLength - bytesDone)) : preBufferLength;
	...
	for (i = 0; i < loopCount; i++)	{
		if (bufLen > 0)	{							// Combining bytes from buffer and preBuffer
		...
		else		{	// Comparing directly from preBuffer
...
**/
@interface GCDAsyncSpecialPacket : NSObject	{  @public	NSDictionary *tlsSettings;	}
- (id)initWithTLSSettings:(NSDictionary *)settings;
@end
@implementation GCDAsyncSpecialPacket
- (id)initWithTLSSettings:(NSDictionary *)settings	{
	return self = super.init ?  tlsSettings = [settings copy], self : nil; }
- (void)dealloc	{	[tlsSettings release];	[super dealloc]; }
@end

@implementation GCDAsyncSocket
- (id)init	{	return [self initWithDelegate:nil delegateQueue:NULL socketQueue:NULL];	}
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq{
	...
	delegate 		= aDelegate;
	if (dq) {		dispatch_retain(dq);		delegateQueue = dq;	}
	...
	else	socketQueue = dispatch_queue_create("GCDAsyncSocket", NULL);
	readQueue 			= [[NSMutableArray alloc] initWithCapacity:5];
	currentRead 		= nil;
	writeQueue 			= NSMutableArray.new;
	currentWrite 		= nil;
	partialReadBuffer = NSMutableData.new;
	return self;
}
- (void)dealloc	{	LogInfo(@"%@ - %@ (start)", THIS_METHOD, self);
	dispatch_get_current_queue() == socketQueue 	? [self closeWithError:nil]
...
- (id)delegate{
	...
	if (dispatch_get_current_queue() == socketQueue)		return delegate;
	__block id result;
...
- (void)setDelegate:(id)newDelegate synchronously:(BOOL)synchronously	{
	...
	dispatch_get_current_queue() == socketQueue 	? 	block()
															  	: 	synchronously
																?	dispatch_sync(socketQueue, block)
																: 	dispatch_async(socketQueue, block);
}
- (void)setDelegate:(id)newDelegate	{	[self setDelegate:newDelegate synchronously:NO];	}
- (void)synchronouslySetDelegate:(id)newDelegate	{	[self setDelegate:newDelegate synchronously:YES]; }
- (dispatch_queue_t)delegateQueue	{

	if (dispatch_get_current_queue() == socketQueue)		return delegateQueue;
...
- (void)setDelegateQueue:(dispatch_queue_t)newDelegateQueue synchronously:(BOOL)synchronously{
	...
	dispatch_block_t block = ^{
		delegateQueue 		? dispatch_release(delegateQueue) : nil;
		newDelegateQueue 	? dispatch_retain(newDelegateQueue) : nil;
...
- (BOOL)isIPv4Enabled{
	...
	if (dispatch_get_current_queue() == socketQueue)	return ((config & kIPv4Disabled) == 0);
	__block BOOL result;
	dispatch_sync(socketQueue, ^{		result = ((config & kIPv4Disabled) == 0); 	});
	return result;
}
...
- (BOOL)isIPv6Enabled{
	...
	if (dispatch_get_current_queue() == socketQueue) return ((config & kIPv6Disabled) == 0);
	__block BOOL result;
	dispatch_sync(socketQueue, ^{	result = ((config & kIPv6Disabled) == 0); 	});
	return result;
}
- (void)setIPv6Enabled:(BOOL)flag	{

	// Note: YES means kIPv6Disabled is OFF
	dispatch_block_t block = ^{	if (flag)	config &= ~kIPv6Disabled;
											else			config |= kIPv6Disabled;		};
	dispatch_get_current_queue() == socketQueue 	?	block()
																:	dispatch_async(socketQueue, block);
}
- (BOOL)isIPv4PreferredOverIPv6{
	...
	if (dispatch_get_current_queue() == socketQueue) return ((config & kPreferIPv6) == 0);
	__block BOOL result;
	dispatch_sync(socketQueue, ^{		result = ((config & kPreferIPv6) == 0); 	});
	return result;
}
...
- (BOOL)acceptOnInterface:(NSString *)interface port:(uint16_t)port error:(NSError **)errPtr{
	...
	int(^createSocket)(int, NSData*) = ^int (int domain, NSData *interfaceAddr) {
		...
		if (status == -1)	{
			err = [[self errnoErrorWithReason: @"Error enabling non-blocking IO on socket (fcntl)"] retain];
			close(socketFD);
			return SOCKET_NULL;
		}
		int reuseOn = 1;
		...
		if (status == -1)
		{
			err = [[self errnoErrorWithReason:@"Error enabling address reuse (setsockopt)"] retain];
			close(socketFD);
			return SOCKET_NULL;
		}
		// Bind socket
		...
		if (status == -1)
		{
			err = [[self errnoErrorWithReason:@"Error in bind() function"] retain];
			close(socketFD);
			return SOCKET_NULL;
		}
		// Listen
		...
		if (status == -1)	{
			err = [[self errnoErrorWithReason:@"Error in listen() function"] retain];
			close(socketFD);
			return SOCKET_NULL;
		}
		return socketFD;
	...
	dispatch_block_t block = ^{	NSAutoreleasePool *pool = NSAutoreleasePool.new;

		if (delegate == nil) { // Must have delegate set
			err = [self badConfigError:@"Attempting to accept without a delegate. Set a delegate first."].retain;
			[pool drain];
			return_from_block;
		}
		if (delegateQueue == NULL) { // Must have delegate queue set

			err = [self badConfigError:@"Attempting to accept without a delegate queue. Set a delegate queue first."].retain;
			[pool drain];
			return_from_block;
		}
		...
		if (!self.isDisconnected) { // Must be disconnected
			err = [self badConfigError:@"Attempting to accept while connected or accepting connections. Disconnect first."].retain;
			[pool drain];
			return_from_block;
		}
		...
		NSMutableData *interface4 = nil, *interface6 = nil;
		[self getInterfaceAddress4:&interface4 address6:&interface6 fromDescription:interface port:port];
		if ((interface4 == nil) && (interface6 == nil))
		{
			err = [self badParamError:@"Unknown interface. Specify valid interface by name (e.g. \"en1\") or IP address."].retain;
			[pool drain];
			return_from_block;
		}
		if (isIPv4Disabled && (interface6 == nil))		{

			err = [self badParamError:@"IPv4 has been disabled and specified interface doesn't support IPv6."].retain;
			[pool drain];
			return_from_block;
		}
		...
		if (enableIPv6)
		{
			...
			{
				if (socket4FD != SOCKET_NULL)	close(socket4FD);
				[pool drain];
				return_from_block;
			}
		}
		...
		if (enableIPv4)
		{
			...
			dispatch_source_set_event_handler(accept4Source, ^{
				NSAutoreleasePool *eventPool = NSAutoreleasePool.new;
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{
			...
			dispatch_async(acceptedSocket->socketQueue, ^{
				NSAutoreleasePool *socketPool = NSAutoreleasePool.new;
...
- (BOOL)connectToHost:(NSString *)host
	...
	dispatch_block_t block = ^{
		...
		dispatch_async(globalConcurrentQueue, ^{
			NSAutoreleasePool *lookupPool = NSAutoreleasePool.new;
...
- (void)didConnect:(int)aConnectIndex{
	...
	if (delegateQueue && [delegate respondsToSelector:@selector(socket:didConnectToHost:port:)])
	{
		...
		dispatch_async(delegateQueue, ^{
			...
			dispatch_async(socketQueue, ^{
				NSAutoreleasePool *callbackPool = NSAutoreleasePool.new;
				SetupStreamsPart2();
				[callbackPool drain];
			});
			[delegatePool drain];
...
- (void)disconnect{
	...
	dispatch_block_t block = ^{
		NSAutoreleasePool *pool = NSAutoreleasePool.new;
		if (flags & kSocketStarted) [self closeWithError:nil];
		[pool drain];
	};
...
- (void)doReadEOF{
	...
	if (shouldDisconnect)
	{
		if (error == nil)	error = [self connectionClosedError];
		[self closeWithError:error];
	}
	else
	{
		if (![self usingCFStream])		[self suspendReadSource]; 			// Suspend the read source (if needed)
	}
}
- (void)completeCurrentRead{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		if (value)
		{
			...
			for (cipherIndex = 0; cipherIndex < numberCiphers; cipherIndex++)
				ciphers[cipherIndex] = [cipherSuites[cipherIndex] shortValue];
			status = SSLSetEnabledCiphers(sslContext, ciphers, numberCiphers);
			if (status != noErr)
				return [self closeWithError:[self otherError:@"Error in SSLSetEnabledCiphers"]];
		}
		// 9. GCDAsyncSocketSSLDiffieHellmanParameters
		...
		if (value)
		{
			...
			if (status != noErr)
				return [self closeWithError:[self otherError:@"Error in SSLSetDiffieHellmanParams"]];
		}
...
- (void)continueSSLHandshake{
	...
	if (status == noErr)
	{
		...
		// Handshake continues... This method will be called again from doReadData or doWriteData.
	else	[self closeWithError:[self sslError:status]];
}
...
- (int)socket4FD	{ return dispatch_get_current_queue() == socketQueue ? socket4FD : SOCKET_NULL; }
- (int)socket6FD	{ return dispatch_get_current_queue() == socketQueue ? socket6FD :  SOCKET_NULL; }
...
- (BOOL)enableBackgroundingOnSocket{
	...
	LogTrace();
	return dispatch_get_current_queue() == socketQueue ? [self enableBackgroundingOnSocketWithCaveat:NO] : NO;
}
- (BOOL)enableBackgroundingOnSocketWithCaveat // Deprecated in iOS 4.???{
	...
	LogTrace();
	return dispatch_get_current_queue() == socketQueue ? [self enableBackgroundingOnSocketWithCaveat:YES] : NO;
}
#else
- (SSLContextRef)sslContext { return dispatch_get_current_queue() == socketQueue ? sslContext : NULL; }
#endif
...
+ (NSString *)hostFromAddress:(NSData *)address	{	NSString *host;

	return [self getHost:&host port:NULL fromAddress:address] ? host : nil;
}
+ (uint16_t)portFromAddress:(NSData *)address	{
	uint16_t port;
	return [self getHost:NULL port:&port fromAddress:address] ? port : 0;
}
